...
def _find_package_prefix(file_names: set[str], required_data_files: tuple[str, ...]) -> str:
    prefixes = []
    ...
    for prefix in prefixes:
        if all(prefix + relative in file_names for relative in ("__init__.py", *(
    ...
    if len(valid_prefixes) != 1:
        raise ArchiveValidationError(
            "Archive must contain exactly one valid Learn Node package root"
        )
...
def extract_and_validate_archive(
    ...
    required_data_files: tuple[str, ...] = REQUIRED_DATA_FILES,
) -> Path:
    ...
    try:
        with zipfile.ZipFile(archive_path) as archive:
            ...
            prefix = _find_package_prefix(file_names, required_data_files)
    ...
    for data_name in required_data_files:
        data_path = package_root / "data" / data_name
        if not data_path.is_file():
            raise ArchiveValidationError(f"Missing required data file: data/{data_name}")
        try:
